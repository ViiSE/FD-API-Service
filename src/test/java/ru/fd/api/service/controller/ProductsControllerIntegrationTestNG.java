...
public class ProductsControllerIntegrationTestNG extends AbstractTestNGSpringContextTests {
    ...
    private Process<Products, List<String>> chain;
    ...
    void whenValidToken_thenReturns200WithContent() throws Exception {
        testMethod("products() [when valid token then returns 200 with content]");
        ...
        ProductsPojo prodPojo = (ProductsPojo) chain.answer(new ArrayList<>());
    ...
    void whenNotValidToken_thenReturns401WithoutContent(String incorrectToken) throws Exception {
        testMethod("products() [when not valid token then returns 401 without content]");
    ...
    void whenNotAuthorizationHeader_thenReturns401WithoutContent() throws Exception {
        testMethod("products() [when not auth header then returns 401 without content]");
    ...
    void whenTokenIsNotJWT_thenReturns401WithoutContent(String notJWTToken) throws Exception {
        testMethod("products() [when token is not JWT then returns 401 without content]");
