...
@Component("psLgCategories")
public class PsLgCategoriesImpl implements Process<Sendable, Void> {

    private final LoggerService logger;
    private final Process<Sendable, Void> process;

    public PsLgCategoriesImpl(
            LoggerService logger,
            @Qualifier("psCategories") Process<Sendable, Void> process) {
        this.logger = logger;
    ...
    public Sendable answer(Void nothing) throws ProcessException {
        try {
            ...
            logger.info(PsLgCategoriesImpl.class, "Site request categories");
            return result;
        } catch (ProcessException ex) {
            logger.error(PsLgCategoriesImpl.class, ex.getMessage() + " <CAUSE>: " + ex.getCause());
            throw ex;
        }
    }
}
