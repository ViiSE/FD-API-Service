...
@Component("psLgDepartments")
public class PsLgDepartmentsImpl implements Process<Sendable, Void> {

    private final LoggerService logger;
    private final Process<Sendable, Void> process;

    public PsLgDepartmentsImpl(
            LoggerService logger,
            @Qualifier("psDepartments") Process<Sendable, Void> process) {
        this.logger = logger;
    ...
    public Sendable answer(Void nothing) throws ProcessException {
        try {
            ...
            logger.info(PsLgDepartmentsImpl.class, "Site request departments");
            return result;
        } catch (ProcessException ex) {
            logger.error(PsLgDepartmentsImpl.class, ex.getMessage() + " <CAUSE>: " + ex.getCause());
            throw ex;
        }
    }
}
