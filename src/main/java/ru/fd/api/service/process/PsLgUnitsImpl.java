...
@Component("psLgUnits")
public class PsLgUnitsImpl implements Process<Sendable, Void> {

    private final LoggerService logger;
    private final Process<Sendable, Void> process;

    public PsLgUnitsImpl(
            LoggerService logger,
            @Qualifier("psUnits") Process<Sendable, Void> process) {
        this.logger = logger;
    ...
    public Sendable answer(Void nothing) throws ProcessException {
        try {
            ...
            logger.info(PsLgUnitsImpl.class, "Site request units");
            return result;
        } catch (ProcessException ex) {
            logger.error(PsLgUnitsImpl.class, ex.getMessage() + " <CAUSE>: " + ex.getCause());
            throw ex;
        }
    }
}
