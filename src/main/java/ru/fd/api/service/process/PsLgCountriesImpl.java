...
@Component("psLgCountries")
public class PsLgCountriesImpl implements Process<Sendable, Void> {

    private final LoggerService logger;
    private final Process<Sendable, Void> process;

    public PsLgCountriesImpl(
            LoggerService logger,
            @Qualifier("psCountries") Process<Sendable, Void> process) {
        this.logger = logger;
    ...
    public Sendable answer(Void nothing) throws ProcessException {
        try {
            ...
            logger.info(PsLgCountriesImpl.class, "Site request countries");
            return result;
        } catch (ProcessException ex) {
            logger.error(PsLgCountriesImpl.class, ex.getMessage() + " <CAUSE>: " + ex.getCause());
            throw ex;
        }
    }
}
