...
@Component("psLgAttributes")
public class PsLgAttributesImpl implements Process<Sendable, Void> {

    private final LoggerService logger;
    private final Process<Sendable, Void> process;

    public PsLgAttributesImpl(
            LoggerService logger,
            @Qualifier("psAttributes") Process<Sendable, Void> process) {
        this.logger = logger;
    ...
    public Sendable answer(Void nothing) throws ProcessException {
        try {
            ...
            logger.info(PsLgAttributesImpl.class, "Site request attributes");
            return result;
        } catch (ProcessException ex) {
            logger.error(PsLgAttributesImpl.class, ex.getMessage() + " <CAUSE>: " + ex.getCause());
            throw ex;
        }
    }
}
