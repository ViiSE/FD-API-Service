...
@Component("psLgVendors")
public class PsLgVendorsImpl implements Process<Sendable, Void> {

    private final LoggerService logger;
    private final Process<Sendable, Void> process;

    public PsLgVendorsImpl(
            LoggerService logger,
            @Qualifier("psVendors") Process<Sendable, Void> process) {
        this.logger = logger;
    ...
    public Sendable answer(Void nothing) throws ProcessException {
        try {
            ...
            logger.info(PsLgVendorsImpl.class, "Site request vendors");
            return result;
        } catch (ProcessException ex) {
            logger.error(PsLgVendorsImpl.class, ex.getMessage() + " <CAUSE>: " + ex.getCause());
            throw ex;
        }
    }
}
