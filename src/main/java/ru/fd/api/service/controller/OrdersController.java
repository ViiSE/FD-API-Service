...
public class OrdersController {
    ...
    public OrderResponsePojo createOrder(
        ...
        try {
            ...
            if(orderResponsePojo.getStatus() == OrderResponses.INTERNAL_SERVER_ERROR) {
    ...
    @ApiOperation(value = "Создает заказ !!!Экспериментальная точка!!!")
    @PostMapping("/orders/dev")
    @ResponseBody
    public OrderResponsePojo createOrderDev(
            @ApiParam(required = true, value = "Поля <code><b>order{id}</b></code>, " +
                    "<code><b>order{city_id}</b></code></pre>, <code><b>order{customer}</b></code>, " +
                    "<code><b>order{delivery}</b></code>, <code><b>order{pay_type_id}</b></code> и " +
                    "<code><b>order{products[]}</b></code> являются обязательными. При их отсутствии API пришлет " +
                    ...
                    ", и статуса заказа " +
                    "<code><b>order_response{status}</b></code>." +
                    "\nЕсли остатков товаров, указанных в запросе, не хватило (то есть остатков каждой единицы товара" +
                    ", указанных в запросе, больше, чем остатков на складах), то ответ также будет содержать в себе " +
                    " список товаров <code><b>order_response{products[]}</b></code>, у которых остатки больше, чем на" +
                    " складах. В списке указывается GID товара " +
                    "<code><b>order_response{products[product{id}]}</b></code> и текущее максимальное количество " +
                    "этого товара на складах. <code><b>order_response{products[product{quantity}]}</b></code>." +
                    "\n___" +
                    "\n<i>Примечание: В данной реализации сайт должен вызвать метод GET /changed_balances сразу после" +
                    " оформления заказа. [TODO: Написать данный GET запрос].")
            @RequestBody OrderPojo orderPojo) {
        try {
            Order order = ordersService.orderCreatorProducer()
                    .getOrderCreatorFromBodyInstance(
